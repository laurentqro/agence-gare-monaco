...
class ImmotoolboxSyncTest < ActiveSupport::TestCase
  setup do
    ...
    # Stub all API endpoints with default data
    stub_districts
    stub_buildings
    stub_properties
    stub_images
  ...
  test "sync creates new buildings from API data" do
    # Create district first so building can reference it
  ...
  test "sync sets multilingual title and description from texts" do
    setup_districts_and_buildings
  ...
  test "sync creates property images" do
    setup_districts_and_buildings
  ...
  def stub_buildings
    stub_request(:get, "#{@base_url}/buildings")
      .to_return(
        status: 200,
        body: [
          { "id" => 10, "name" => "Le Millefiori", "nameAlt" => nil, "address" => "1 Rue du Ténao", "city" => "Monaco", "district" => { "id" => 1 } }
        ].to_json,
        headers: { "Content-Type" => "application/json" }
      )
  end
    ...
    stub_request(:get, "#{@base_url}/texts")
      ...
      .to_return(
        ...
        body: [
          ...
          { "id" => 301, "language" => "en", "title" => "Studio Monte-Carlo EN", "description" => "Beautiful studio" }
        ].to_json,
        headers: { "Content-Type" => "application/json" }
      )
  end
